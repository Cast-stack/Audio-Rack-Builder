...
export const SEED_DEVICES: SeedDevice[] = [
  ...
  },
  {
    ...
    passive: false,
    description:
      "1U Merit-series conditioner with two pull-out swivelling rack lights on a dimmer: eight switched rear outlets, one on the front, 15 A capacity on a 6 ft captive Edison cord.",
    formFactor: "full-rack",
    rackUnits: 1,
    depthMm: 191,
    depthIsOverall: true,
    ...
    status: "current",
    statusNote: null,
    productUrl: "https://furmanpower.com/products/m-8lx",
    datasheetUrl: FURMAN_DATASHEET,
    ports: [
      { label: "AC In", connector: "NEMA 5-15P", direction: "input", signal: "power", channels: null, count: 1, face: "rear", projectionMm: null },
      { label: "Switched outlets", connector: "NEMA 5-15R", direction: "output", signal: "power", channels: null, count: 8, face: "rear", projectionMm: null },
    ...
    unresolved: [
      "powerMaxW covers the two lamps only. It is the one consumption figure Furman prints for this unit; the filtering circuit's own draw is not published and is not counted. Unlike its 15 A output rating, which is what the conditioner passes rather than what it uses.",
      "panel.front. Furman describe the lamps, dimmer and front outlet in prose rather than a numbered callout drawing, so the face is drawn from the category template.",
      "depth behind rails (manufacturer prints overall depth only)",
    ],
    provenance: [
      { field: "rackUnits", sourceUrl: FURMAN_DATASHEET, quote: "Dimensions 1.75\" H x 19\" W x 7.5\" D (M-8Lx/M-8Dx)", confidence: 0.9, derivation: "Printed height 1.75 in = 44.45 mm, one rack unit exactly. The datasheet prints no RU figure." },
      { field: "depthMm", sourceUrl: FURMAN_DATASHEET, quote: "Dimensions 1.75\" H x 19\" W x 7.5\" D (M-8Lx/M-8Dx)", confidence: 0.8, derivation: "7.5 in x 25.4 = 190.5 mm, rounded to 191. TAKEN FROM THE M-8Lx/M-8Dx LINE, NOT THE M-8x2 ONE: the lamp fixtures make this twice as deep as its lampless sibling, 7.5 in against 3.75 in. Printed OVERALL depth." },
      { field: "weightLb", sourceUrl: FURMAN_DATASHEET, quote: "Weight M-8Lx/M-8Dx: 6 lbs. (2.7 kg)", confidence: 0.95, derivation: "Read directly. 2.7 kg x 2.20462 = 5.95 lb, consistent with the printed 6." },
      { field: "powerMaxW", sourceUrl: FURMAN_DATASHEET, quote: "Lamps (M-8Lx/M-8Dx) Two 120V incandescent bulbs, 4 watts each", confidence: 0.7, derivation: "DERIVED. 2 x 4 W = 8 W with both lamps out and up. Lamps only; Furman publish no figure for the rest of the unit." },
      { field: "ports", sourceUrl: FURMAN_DATASHEET, quote: "the eight switched outlets in the rear panel will power up and protect all your equipment up to a 15-amp load | With a 15A capacity and nine total outlets | Line Cords 6 ft. captive, 14 AWG, with 3-conductor Edison Plug", confidence: 0.75, derivation: "Eight switched rear outlets stated; nine total less eight rear puts the ninth on the front. The captive Edison cord is the AC input, so there is no inlet to plug into." },
      { field: "description", sourceUrl: FURMAN_DATASHEET, quote: "The M-8Lx and M-8Dx feature two slide out, swiveling light fixtures to provide discreet illumination of your equipment on stages, in studios, and in other dark places. A dimmer knob lets you adjust the lamp brightness to just the right level. | Maximum Output Current 15 Amps | Noise Attenuation (Transverse Mode) M-8Lx: Greater than 26db, 200Khz to 10MHz", confidence: 0.9, derivation: "Lamp and dimmer description and ratings read from the datasheet." },
    ],
  },

  {
    ...
    passive: false,
    description:
      "1U Merit-series conditioner with two pull-out rack lights and a three-digit LED voltmeter reading incoming line voltage: eight switched rear outlets, one on the front, 15 A capacity.",
    formFactor: "full-rack",
    rackUnits: 1,
    depthMm: 191,
    depthIsOverall: true,
    ...
    status: "current",
    statusNote: null,
    productUrl: "https://furmanpower.com/products/m-8dx",
    datasheetUrl: FURMAN_DATASHEET,
    ports: [
      { label: "AC In", connector: "NEMA 5-15P", direction: "input", signal: "power", channels: null, count: 1, face: "rear", projectionMm: null },
      { label: "Switched outlets", connector: "NEMA 5-15R", direction: "output", signal: "power", channels: null, count: 8, face: "rear", projectionMm: null },
    ...
    unresolved: [
      "powerMaxW covers the two lamps only, and this model also carries a voltmeter the M-8Lx does not. Furman print no figure for the voltmeter or the filtering circuit, so 8 W is a floor for this unit rather than a ceiling — the one place in this catalog where powerMaxW is known to understate.",
      "panel.front. Furman describe the lamps, dimmer, voltmeter and front outlet in prose rather than a numbered callout drawing, so the face is drawn from the category template.",
      "depth behind rails (manufacturer prints overall depth only)",
    ],
    provenance: [
      { field: "rackUnits", sourceUrl: FURMAN_DATASHEET, quote: "Dimensions 1.75\" H x 19\" W x 7.5\" D (M-8Lx/M-8Dx)", confidence: 0.9, derivation: "Printed height 1.75 in = 44.45 mm, one rack unit exactly." },
      { field: "depthMm", sourceUrl: FURMAN_DATASHEET, quote: "Dimensions 1.75\" H x 19\" W x 7.5\" D (M-8Lx/M-8Dx)", confidence: 0.8, derivation: "7.5 in x 25.4 = 190.5 mm, rounded to 191. The M-8Lx/M-8Dx line, not the M-8x2 line. Printed OVERALL depth." },
      { field: "weightLb", sourceUrl: FURMAN_DATASHEET, quote: "Weight M-8Lx/M-8Dx: 6 lbs. (2.7 kg)", confidence: 0.95, derivation: "Read directly. 2.7 kg x 2.20462 = 5.95 lb, consistent with the printed 6." },
      { field: "powerMaxW", sourceUrl: FURMAN_DATASHEET, quote: "Lamps (M-8Lx/M-8Dx) Two 120V incandescent bulbs, 4 watts each", confidence: 0.55, derivation: "DERIVED AND KNOWN INCOMPLETE. 2 x 4 W = 8 W for the lamps. The three-digit LED voltmeter on this model draws as well and Furman do not print how much, so this figure is a floor. Confidence is set low to say so." },
      { field: "ports", sourceUrl: FURMAN_DATASHEET, quote: "the eight switched outlets in the rear panel will power up and protect all your equipment up to a 15-amp load | With a 15A capacity and nine total outlets | Line Cords 6 ft. captive, 14 AWG, with 3-conductor Edison Plug", confidence: 0.75, derivation: "Eight switched rear outlets stated; nine total less eight rear puts the ninth on the front." },
      { field: "description", sourceUrl: FURMAN_DATASHEET, quote: "The M-8Dx adds a bright red, three-digit LED voltmeter that displays incoming voltage. | Maximum Output Current 15 Amps | Operating Voltage 120VAC 60Hz | Maximum Continuous Operating Voltage 130 VAC RMS", confidence: 0.9, derivation: "Voltmeter description and ratings read from the datasheet." },
    ],
  },

  {
    id: "tripplite-smart1500rm2u",
    slug: "tripp-lite-smart1500rm2u",
    brand: "Tripp Lite",
    model: "SMART1500RM2U",
    category: "UPS",
    passive: false,
    description:
      "2U line-interactive UPS, 1500 VA / 1350 W at 120 V, with pure sine wave output on eight 5-15R outlets, automatic voltage regulation, hot-swappable batteries and an LCD.",
    formFactor: "full-rack",
    rackUnits: 2,
    depthMm: 342,
    depthIsOverall: true,
    ...
    poePowered: false,
    status: "current",
    statusNote: "Sold by Eaton under the Tripp Lite series name.",
    productUrl:
      "https://tripplite.eaton.com/smartpro-120v-1-5kva-1-35kw-line-interactive-sine-wave-ups-2u-rack-tower-snmp-card-option-lcd-usb-db9-8-outlets~SMART1500RM2U",
    datasheetUrl: "https://assets.tripplite.com/product-pdfs/en/smart1500rm2u.pdf",
    ports: [
      { label: "AC In", connector: "NEMA 5-15P", direction: "input", signal: "power", channels: null, count: 1, face: "rear", projectionMm: null },
    ...
    unresolved: [
      ...
      "powerMaxW is the unit's OWN losses at full load, derived from the heat it sheds. It is not what passes through it: that is up to 12 A, and whatever is plugged into the UPS has to be budgeted separately.",
      "inrushFactor is the house default of 1.5, not a Tripp Lite figure. A UPS charging a flat battery at power-up can pull harder than that, and this is the kind of unit the inrush check exists for.",
      "panel.front. The datasheet lists the LCD, its buttons and the battery access door in prose but numbers no callouts, so the face is drawn from the category template.",
      "powerTypicalW",
    ],
    provenance: [
      { field: "rackUnits", sourceUrl: "https://assets.tripplite.com/product-pdfs/en/smart1500rm2u.pdf", quote: "Primary Form Factor Rackmount | Rack Height 2U | Primary UPS Height (mm) 88", confidence: 0.95, derivation: "Printed as 2U directly, and 88 mm is two rack units (2 x 44.45 = 88.9 mm), so the two agree." },
      { field: "depthMm", sourceUrl: "https://assets.tripplite.com/product-pdfs/en/smart1500rm2u.pdf", quote: "Primary UPS Depth (mm) 342 | Unit Dimensions (hwd / in.) 3.750 x 17.300 x 13.450 | Minimum Required Rack Depth (inches) 17 | Minimum Required Rack Depth (cm) 43.18", confidence: 0.85, derivation: "342 mm is the chassis and is what is recorded. Note the manufacturer separately requires a 17 in / 432 mm rack, which is 90 mm more than the box — a rare case of a maker publishing the clearance rather than leaving it to be worked out." },
      { field: "weightLb", sourceUrl: "https://assets.tripplite.com/product-pdfs/en/smart1500rm2u.pdf", quote: "Unit Weight (lbs.) 46.5000 | Unit Weight (kg) 21.09", confidence: 0.95, derivation: "Read directly. 21.09 kg x 2.20462 = 46.5 lb, matching. Batteries included; this is the heaviest unit in the catalog and it belongs at the bottom of a rack." },
      { field: "powerMaxW", sourceUrl: "https://assets.tripplite.com/product-pdfs/en/smart1500rm2u.pdf", quote: "AC Mode BTU / Hr. (Full Load) 267 | AC Mode Efficiency Rating (100% Load) 95% | Output Capacity (Watts) 1350", confidence: 0.6, derivation: "DERIVED, NOT PRINTED AS WATTS. 267 BTU/hr / 3.412 = 78.2 W, rounded to 78. Heat shed at full load is the unit's own loss, which is what a rack has to budget for the UPS itself. The 95% efficiency figure corroborates it: 5% of 1350 W is 68 W, the same order." },
      { field: "ports", sourceUrl: "https://assets.tripplite.com/product-pdfs/en/smart1500rm2u.pdf", quote: "Output Receptacles (8) 5-15R | UPS Input Connection Type 5-15P | Maximum Input Amps 12 | Nominal Input Voltage(s) Supported 120V AC | The 10-foot power cord with NEMA 5-15P plug connects to any compatible AC outlet. | RS-232 and HID-compliant USB ports connect to a computer running free downloadable PowerAlert software", confidence: 0.9, derivation: "Outlets, inlet and data ports read from the specification table and the feature text. The captive 10 ft cord is the AC input; there is no inlet." },
      { field: "description", sourceUrl: "https://assets.tripplite.com/product-pdfs/en/smart1500rm2u.pdf", quote: "Output Capacity (VA) 1500 | Output Capacity (Watts) 1350 | Two switchable two-outlet 5-15R load banks | Automatic Voltage Regulation (AVR) Corrects Low- and High-Voltage Conditions | The field-replaceable, hot-swappable VRLA battery modules | UPS Housing Material Steel | Cooling Method Fan", confidence: 0.9, derivation: "Capacity, regulation, battery and construction read from the datasheet." },
    ],
  },
];
